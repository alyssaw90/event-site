        ...
        .post(isLoggedInAdmin, (req, res) => {
            models.sql.sync()
                ...
                .then(function(user) {
                    if (user) {
                        res.status(419).json({
                            msg: 'email address alread in use'
                        });
                    }
                    if (!user) {
        ...
        .patch(isLoggedIn, (req, res) => {
            models.sql.sync()
                ...
                .then((user) => {
                    ...
                    delete req.body.password;
                    if (hasNewPw) user.update({
                        password: newPw
                    });
                    newPw = undefined;
                    if (req.body.name) user.update({
                        userName: req.body.name
                    });
                    if (req.body.email) user.update({
                        email: req.body.email
                    });
                    if (req.body.isAdmin === true || req.body.isAdmin === false) user.update({
                        isAdmin: req.body.isAdmin
                    });
                    res.json({
                        msg: `user updated`
